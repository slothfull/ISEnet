...
# -*- coding:utf-8 -*-
"""
@time:2019/3/9 下午9:47
@author:bigmelon
todo build some summaries classes for visualize a given tensor in default graph
todo merge is used with "one-feed" | if multi-feed => go with one by one!

todo -> excitation-statistic-class 仍然存在 max_open_warning 的问题

todo -> save the excitation tensor values as numpy array for data analysis!!!!!
"""
...
from q_tensor_name_lists_50 import se_chosen_excitations, sese_chosen_excitations, \
...
class PrecisionSummary:
    ...
    def __init__(self, session, writer, process_flag, num_batches, restore=False):
        ...
        self.sveb_fp = float(0)  # 不是s被分成s的数量
        self.recall = float(0)
        self.ppr = float(0)
    ...
    def push(self, epoch):  # total train/valid num batches
        ...
        if self.process_flag == 'test':
            ...
            self.ppr = self.sveb_tp/(self.sveb_tp + self.sveb_fp)
        else:
            pass
...
class ExcitationSummary:
    ...
    def push(self, sample_lst, xs, epoch):
        ...
        if epoch % 50 == 0 and epoch != 0:
            self.__save_tensor_for_statistics(excitation_2, 1, 3, epoch)
            self.__save_tensor_for_statistics(excitation_6, 2, 4, epoch)
            self.__save_tensor_for_statistics(excitation_12, 3, 6, epoch)
            self.__save_tensor_for_statistics(excitation_15, 4, 3, epoch)
        # write summaries
        self.__excitation_line_chart(excitation_2, 1, 3)  # [n:(1,1,1,x) s:(1,1,1,x)...]
        ...
        self.writer.add_summary(summary, epoch)
        self.__excitation_line_chart(excitation_6, 2, 4)  # [n:(1,1,1,x) s:(1,1,1,x)...]
        self.__fig_to_rgb_array()
        ...
        self.writer.add_summary(summary, epoch)
        self.__excitation_line_chart(excitation_12, 3, 6)  # [n:(1,1,1,x) s:(1,1,1,x)...]
        self.__fig_to_rgb_array()
        ...
        self.writer.add_summary(summary, epoch)
        self.__excitation_line_chart(excitation_15, 4, 3)  # [n:(1,1,1,x) s:(1,1,1,x)...]
        self.__fig_to_rgb_array()
