...
from q_tensor_name_lists_14 import se_chosen_excitations, sese_chosen_excitations, \
...
class ExcitationSummary:
    ...
    def push(self, sample_lst, xs, process_flag, epoch):
        """
        ...
        for i, xi in enumerate(self.sequenced_lst):  # nsvfq
            feed_dict = {xs: xi, process_flag: False}  # todo !!
        ...
        if epoch % 50 == 0 and epoch != 0:
            ...
            self.__save_tensor_for_statistics(excitation_6, 2, 1, epoch)
            self.__save_tensor_for_statistics(excitation_12, 3, 1, epoch)
        ...
        self.writer.add_summary(summary, epoch)
        self.__excitation_line_chart(excitation_6, 2, 1)  # [n:(1,1,1,x) s:(1,1,1,x)...]
        self.__fig_to_rgb_array()
        ...
        self.writer.add_summary(summary, epoch)
        self.__excitation_line_chart(excitation_12, 3, 1)  # [n:(1,1,1,x) s:(1,1,1,x)...]
        self.__fig_to_rgb_array()
